...
# Rev 12 — 13 August 2026 · the bill and the kitty, redrawn

Cumulative on rev 11 and everything before it. Board: `design/Bill and Kitty.dc.html`. Build reference: `screens-bill-kitty.html`. Full spec: `11-bill-and-kitty.md`.

| # | Change | Status |
|---|---|---|
| S54 | **The B-series bill screens are dead.** B1–B6 on the night board are replaced by six screens in the Tonight chrome: **L1** the bill · **L2** add a spend · **L3** spend / edit · **L4** nothing on it · **L5** bill rules · **L6** kitty rules. | FINAL |
| S55 | **Nothing on the bill screens calculates a share.** While the game runs no result exists, so the screens carry the spends and the formula only. Winner shares, reimbursement amounts and the kitty total are computed once, at settle-up, against the counted table. | FINAL |
...
| S58 | **Covered by has four cases:** one player · several players, whose fronted amounts must sum to the spend or Save is blocked · the kitty, which takes no reimbursement · nobody yet, which leaves the spend unpaid with an amber tag. | FINAL |
| S59 | **The kitty is not on the bill.** It is its own screen off the house rules: on/off for tonight, the charge, **who collects** it, and a per-player off-tonight exception where the filled chip is the player switched off. | FINAL |
