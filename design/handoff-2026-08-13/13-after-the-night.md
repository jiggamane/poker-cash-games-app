...
| Player | In | Out | Gross | Bill | Back | Kitty | Net |
|---|---|---|---|---|---|---|---|
...
- Nets sum to **−126**: the kitty is the only money that leaves the table. The $170 bill returns to
  the two people who fronted it.
- Transfers on E4, six of them: Petr → Dana 1,230 · Ivo → Marek 500 · Ivo → Lena 280 ·
  Tomáš → Dana 252 · Tomáš → Lena 122 · Tomáš → the kitty 126. Each payer's outgoings equal their net;
...
kitty land at the next step" and "Ranks are provisional until every stack is counted." One action:
**Back to the count**.
...
- Header card: total leaving the table, with its destinations on one line —
  `$296 · $120 back to Marek, $50 to Lena · $126 to the kitty`.
- One block per rule: name and total, the split sentence, then a per-player row with a pencil. The
  kitty block shows its working (`5% of $1,620 → $81`) because a percentage is not checkable otherwise.
- **Everyone after deductions is a preview**: dashed frame, `PREVIEW` tag, columns GROSS / BILL / BACK
  / KITTY / NET, recomputed whenever a figure above changes. Losers show gross and net only — both
...
- Header reads `$126 · Only the kitty so far · the bill is not in yet`.
- Preview BILL and BACK are em dashes; NET is the kitty-only figure.
...
- A spend added later — including during settle-up — recalculates every share and transfer, and the
  settlement screen must say it changed (`11-bill-and-kitty.md`, S61).

## E4 · Settle up

- Transfer row: payer, arrow, payee, amount. **The kitty is a payee like anyone else.**
...
- Third route, in the footnote: write the difference off to the kitty. That creates a real ledger
...
and for the night just closed. 1C's three-figure summary, its coloured `in / out / bill / kitty` tokens
...
(`Kitchen & drinks → Marek, Lena $170` / `Kitty · held by the group $126`). Its figures are correct;
...
- The **write-off-to-the-kitty** confirmation from E5.
