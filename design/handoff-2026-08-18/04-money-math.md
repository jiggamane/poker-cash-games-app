...
**Group piggy bank · 5% of the gross win**
...
| Winner | Gross | Piggy bank | Bill | Net |
| --- | --- | --- | --- | --- |
...
**$296 leaves the table**: $170 to the bill, $126 to the piggy bank. Losers are untouched — both rules charge winners only, so Tomáš, Ivo and Petr owe exactly their gross loss.
...
| | Gross | Piggy bank | Bill share | Fronted | Net |
| --- | --- | --- | --- | --- | --- |
| Dana | +1,620 | −81 | −57 | — | **+$1,482** |
...
1. For every participant, `net = counted − total money in`, then apply every deduction charged to them. Collectors and the group's own destinations (piggy bank, host fee, next pot) enter as additional creditors. Nets must sum to zero.
...
The same rules run wherever money is divided: bill splits, piggy bank percentages, per-player deductions and the settle-up preview on B4.
...
The collector field therefore means "who holds money for rules that accumulate" — the piggy bank, a host fee — and never "who gets the bill back". A bill has fronters, not a collector.
...
### 3. Is Lena in the piggy bank or not?

O7 and B6 show Lena switched off the piggy bank for tonight ("off tonight — she brought the food"), leaving 3 of 5 paying in. E3 charges her $22 of piggy bank. Those screens were drawn from different sample nights — O7/B6 also list five at the table without Dana, who plays and wins in E2.
