...
| The night, the bill and the piggy bank | `design/Screens - The night.dc.html` | N1–N10, B1–B6 |
...
**H1 · first run.** Hatched card: "Nothing on the book yet" / "Your first session" / an explanation of what the app does. Primary **Open the table**. Below: Add the players (with "nobody in the group yet"), Money rules ("optional · bill, piggy bank, host fee"), Settings. Foot: Invite a player.
...
**G4 · My stats.** Segmented: All groups / per group. Hero net, then best night, worst night, nights in the black, then a per-group breakdown. Every figure is after piggy bank and bills. Footnote states the privacy rule: only you see this page; inside a group, members see your net in that group and nothing else.
...
**C2 · Players.** Regulars and Occasional, each row an avatar initial, name, meta ("12 nights · in the group since Jan", "8 nights · watcher", "2 nights · not invited yet") and all-time net in green or red. Header note: nets are all-time, after the piggy bank. Actions: Add a player · Invite.

**C5 · Edit a player.** Name field, "Pays into the piggy bank" toggle, nights played, a link into their history. The two paragraphs about renaming and removing are load-bearing — keep them verbatim. Destructive action **Remove from the group** ("no longer seated"), outline only, never filled. An open debt shows as a warning on this screen and never blocks removal.
...
**C6 · The piggy bank · who pays in.** The group-level default. Rate shown with a Change control. A list of members with a pays-in toggle each; the host is marked, the house is marked "never pays in", a guest "plays free". Counter reads "Pays in · 4 of 6". Toggle: new players pay in by default.
...
**O3 · Money rules · empty.** "No rules yet" with the explanation of what a rule does and the reassurance that most clubs set these once. Then three starting points — Food & drinks, Group piggy bank, Host fee — each with a USE action. Primary **Add the first rule**.

**O4 · Money rules · configured.** An untitled rule reads "Set it up · Nothing set yet — the tags fill in as you choose." Configured rules are cards: name, tags (`FIXED · $170`, `SPLIT BY WINNERS`, `COLLECTED BY MAREK`), and a plain-language line. The live preview sits above the primary: "If tonight ends like last Tuesday, $296 leaves the table: $120 back to Marek, $50 to Lena, $126 to the piggy bank." Primary names the act and the time: **Open the table · 20:05**.
...
**O7 · The piggy bank · tonight.** Tonight-only overrides. Rate restated as a group rule. Each person: on, "off tonight" with a reason, or "never pays in · group setting". Counter "Pays in tonight · 3 of 5". Note that it can be changed again mid-night from the house rules sheet. Primary **Use these**.
...
## 07 · The bill and the piggy bank

**B1 · House rules sheet.** Reached from the live session. Each rule with its current figure — the bill at its actual total, the piggy bank as an estimate ("≈ $126"), each with a plain line beneath. "Comes off 3 of 5 tonight · Change" opens B6. Actions: Add an expense · Done.
...
**B6 · Piggy bank · change it mid-night.** Tonight only, changes who it comes off and not the rate. Per person toggles with reasons. Note: nothing already logged changes — the piggy bank is worked out at settle-up. Primary **Save for tonight**.
...
**E3 · Deductions (2 of 3).** Hero: **Leaves the table $296** with the destinations spelled out ("$120 back to Marek, $50 to Lena · $126 to the piggy bank"). Then one block per rule: name, total, an explanatory line ("Split equally between the winners · Marek fronted $120, Lena $50. Tap a figure to change it."), and a row per charged player. For a percentage rule each row shows its working ("5% of $1,620 · $81").

Foot: **Everyone after deductions** — a preview of the night's result, framed as provisional: a dashed outline, a `PREVIEW` tag and a line explaining that it updates as the figures above are set. It holds one row per player with every component of the arithmetic in its own labelled column: **Gross · Bill · Back · Piggy bank · Net**. Nothing is collapsed into a single expression. The two deduction columns carry a bone wash running the height of the table so a reader can follow one kind of money down the list; reimbursements ("Back") sit plain in ink, since that money returns to a person rather than leaving the table. Green and red stay on the net alone. Primary **See who pays whom**.

**E3b · Deductions · before the bill.** The same screen before every figure is in. The bill reads "—" with "Nothing on the bill yet. Add it and the split appears here." and its per-person rows show em dashes; the piggy bank, being a rate, already computes. In the preview the Bill and Back columns are em dashes, the Piggy bank column is filled, and each net is gross minus piggy bank (Dana +$1,539, Marek +$437, Lena +$408). The hero reads $126 · "Only the piggy bank so far · the bill is not in yet", and the note becomes "Waiting on the bill. The count fills in as the figures above are set."
...
**E4 · Settle up (3 of 3).** One line of context: "Six transfers clear the night. The piggy bank is set aside for the group." Then the transfers: payer → payee → amount, with rule destinations ("The piggy bank") as payees. Below, Night's net per player, sorted winners first, tinted rows. Actions: **Close the session** · Share · Export. Edit in the nav bar opens the editing mode described in `04-money-math.md`.

**E5 · Settle up · out of balance.** Replaces E4 when the count does not agree. "It doesn't add up · Off by $150", the arithmetic in words, then every player with in, out and result. The resolution line names three ways forward: fix a count, add the missing buy-in, or write the difference off to the piggy bank. Actions: **Settle the night** · Fix.

**E6 · Night settled.** The archived night: date, times, duration, player count, "settled". Three figures: through the table, entries, off the table. Net after deductions per player with in/out beneath each name. Then the destinations ("Kitchen & drinks → Marek $170", "Piggy bank · held by the group $126"). Actions: **Full ledger** · Rematch. Share in the nav bar.
