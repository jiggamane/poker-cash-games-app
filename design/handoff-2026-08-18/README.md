...
**Building? Start with `CC-HANDOUT.md`.** It is organised as the four design boards — club and
people · the night · settle and the book · not-in-the-build — with build order per part, the rules
that are easy to get wrong, and the open comments with their status. `15-screen-geometry.md` binds
every screen: the reference device is **393 × 852**, the standard iPhone, and the sheet spec, gaps
and rules for other resolutions live there.

**`CHANGELOG.md` — read it before anything else.** It is cumulative and **nothing in it has been applied yet**: rev 15 (the invite and watcher set — X1 is a push, not a root; the dead-invite line is one string for four causes), rev 14 (the ending flow, E1–E7), rev 13 (the group), rev 12 (bill and piggy bank), rev 11 (N3–N10), rev 10 (My stats uses G4; the night-results screen is 1C, replacing E6), rev 9 (navigation settled — no tab bar, push + sheet), rev 8 (the session screen rebuilt as one list, one dock, one card per player), then rev 7 (in-game nav, rebuy default), then the 12 August delta. Each section names what it supersedes and carries the answers to the open money-rule questions.

**Where a spec file and `02-screens.md` disagree, the numbered spec wins.** Files `08` through `14` were written after `02` and each names what it supersedes: `08` the session screens, `09` the navigation table, `10`–`14` the screens they cover.
...
| `15-screen-geometry.md` | **Frame, safe areas, the sheet object, gaps, other resolutions, acceptance checks.** |
| `CC-HANDOUT.md` | **The build handout, by app part.** Where to start, in what order, with the open comments. |
...
| `09-navigation.md` | **Navigation, final.** Push chrome, sheet chrome, depth limits, and every screen classified. |
| `10-screens-N3-N10.md` | The player page and the rest of the in-night screens. |
| `11-bill-and-piggy-bank.md` | The bill and the piggy bank: entry, splits, rules, and what happens at close. |
| `12-the-group.md` | The group: roster, settings, money rules, stats. |
| `13-after-the-night.md` | **The ending flow, E1–E7.** Counting up, settlement, the final tally. |
| `14-invite-and-watcher.md` | **Invites and watchers, X2 / C3 / X1.** Claiming a place, issuing a code, watching read-only. |
...
The designs include a membership tier system — Free, Regular at $2.49/mo, Full at $9.99/mo (the older $4 / $9 in `01-product-logic.md` § Membership is superseded) — drawn across screens P1–P5, S1, G5. **Build none of it.** Build the screens' *content* where they are also useful without tiers (the sessions list, my stats), and skip the tier, plan, upsell and locked-state screens entirely.

**One exception is drawn but equally out of scope: the plan block on X2b** (`14-invite-and-watcher.md` § X2b). It is the only upsell in any drawn screen, it was drawn at the client's request, and it is **drawn but not scheduled** — X2b ships without it. Nothing paid is ticked by default there, and the claim is identical whichever row is selected, so removing the block leaves a working screen: the card, the three statements, and a primary reading "Claim your place".
...
**The later sets ship as flat `screens-*.html` files at the top of this folder, and those are the spec.** Each pairs with its numbered document, needs no `support.js`, and opens in any browser. Every dimension, weight and colour is inline on the element — read the markup rather than measuring the picture. Beside each set of frames is a notes column carrying state, blocking issues and the copy to preserve.

| Drawing | Document | Screens |
| --- | --- | --- |
| `screens-tonight-home.html` | `08` | H1–H5, the live night |
| `screens-N3-N10.html` | `10` | N3–N10 |
| `screens-bill-piggy-bank.html` | `11` | B1–B6 |
| `screens-group.html` | `12` | C1–C6, G1–G5 |
| `screens-after-the-night.html` | `13` | E1–E7 |
| `screens-invite-and-watcher.html` | `14` | X2, C3, X1 |
...
Where a drawing had to invent a string to be drawable at all, it is marked **PROPOSED** in that document's strings table and is awaiting sign-off — treat those as drafts, not decisions. `14` carries the largest set of them. One is not a copy preference but a security property: **the dead-invite line is one string for four causes** (unknown, spent, revoked, expired), and it comes with three build requirements — one error shape on the wire, constant response time across all four, and no client-reachable cause in logs. `14-invite-and-watcher.md` § 3 has the detail.
...
2. **O7/B6 have Lena opted out of the piggy bank tonight, but E3 charges her $22 of it.** Those screens are drawn from different sample nights. E2/E3/E4/E6 are one self-consistent night and are the authoritative example.

3. **The bill split.** The E-series frames draw the older even split (57 / 57 / 56); S62 changed it to by-size-of-win, and `14-invite-and-watcher.md` § 5 carries the re-derivation (110 / 31 / 29, nets summing to −126). **Take the figures in `14` and treat the E-series numbers as stale.** Neither rule name may be hard-coded — it comes from the session snapshot.

A fourth disagreement — who is paid back for the bill — has since been **decided and redrawn**: each person who fronts part of a bill is reimbursed exactly what they fronted, and still pays their own share. B4, E3, E4 and E6 all reflect this. The reasoning and the arithmetic are in `04-money-math.md`.
