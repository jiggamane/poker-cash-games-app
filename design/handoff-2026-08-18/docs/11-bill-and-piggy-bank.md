# 11 · The bill and the piggy bank

Rev 12, 13 August 2026. Six screens, drawn on `design/Bill and Piggy Bank.dc.html` and rendered for build in
`screens-bill-piggy-bank.html`. This document **replaces** the B-series bill screens (`B1`–`B6`) in
...
**L5** bill rules · **L6** piggy bank rules.
...
winner is known, no per-player amount can exist and the piggy bank's take is unknowable. The bill screens
...
states the piggy bank rule with no figure. If you find yourself needing a winner's name before the count,
...
- **Covered by** — chips: each seated player, `The piggy bank`, and a dashed `Nobody yet`.
...
| The piggy bank | The piggy bank paid directly. Nobody is reimbursed; the money left the piggy bank. |
...
Rows: **Piggy bank on tonight** with a switch (48 × 29, radius 15, green when on, 23px white knob),
