...
| # | Change | Status |
|---|---|---|
| S87 | **Reference device is the standard iPhone, 393 × 852.** Every frame was drawn at 402 × 874 (iPhone 16 **Pro**), which quietly gave layouts 22 points they will not have on the base device. All 130+ frames retargeted; bezels 415 wide; caption columns follow. Approvals from here on are against the base device, and larger phones are covered by the spacer rule. | FINAL |
| S88 | **Sheet geometry, gaps and the pinned footer block are specified to the point** in `15-screen-geometry.md` §3: radius 26, grabber 38 × 5 at 9/2, header 12/22 with 8 below a subhead and 14 below none, body rows at 22 and card stacks at 20, footer 14/20/6 with a 17-point button, indicator band 22. Reserved bottom block is 82. **A sheet never scrolls as a whole** — it promotes to full-height and scrolls its body. | FINAL |
| S89 | **Four screens overflowed at 852 and were fixed by tightening, not by cutting content.** `E3`/`E3b` deductions preview: row padding 5 → 3, block 10/9 → 8/7 (was 29 over, Confirm was off-screen). `C7` settlement due: option rows 14 → 11, plain-words card 16 → 13 (19 over). `P1` membership: tier cards 18 → 15, note 16 → 13 (10 over). Measured clean afterwards on all four boards. | FINAL |
| S90 | **Rules for other resolutions are written** (doc 15 §4, with a worked table for SE 3, 13 mini, 16, 16 Pro, 16 Pro Max): type and side margins never scale, vertical difference goes to the single flexible spacer, lists scroll and sheets fit, the footer block is always 82 and always pinned, below 700 points of height every peek sheet promotes to full-height. Row height is identical across devices; only the number of visible rows changes. | FINAL |
| S91 | **The light theme is complete.** 24 frames had no light twin and now do: GR2, GR3, GR5, GR6, GR7, GR8, L5, L6, C3b, C3c, C3e, X2a, X2d (part 1) · H3, H3b, H4, H5, X1b, L3, L4 (part 2) · 1A, 1B, 1C, 1D (part 3). Generated from the dark originals through the token map derived from the 49 existing pairs, so the mapping is the one already approved. | FINAL |
| S92 | **66 light frames drew the home indicator white on white.** Fixed to `rgba(12,13,15,.85)` across the boards and the reference files. Added to the acceptance checks. | FINAL |
| S93 | **"Kitty" is now "piggy bank" everywhere** — 54 files, boards, docs and published references. Screen labels follow (`O7 The piggy bank · tonight`, `L6 Piggy bank rules`). | FINAL |
| S94 | **Money rules exist at two levels.** The group setting is the default a game opens with, pre-selected, changeable for that game alone from the game's own money rules; changing the group default never rewrites a game that is running or already settled. Stated on C7. | FINAL |
| S95 | **`O4` money rules is one hairline list**, not a stack of cards: "Tonight's rules" caption, title with chevron, one detail line, toggle right, "Add a rule" as the last row. The **"Untitled rule" placeholder is removed** — an unconfigured rule does not exist as a row. Brings O4 in line with O3. | FINAL |
| S96 | **E3 and the screen after it read "Confirm"** on the primary, replacing "See who pays whom". | FINAL |
| S97 | **A build handout exists: `CC-HANDOUT.md`**, organised as the four boards, with build order and the rules that are easy to get wrong per part. It carries the open comments too: the **group switch** for admins and multi-group players (top of home, not a settings line) is the next design task; the **keypad-up state** for money entry is specified but not drawn; two instructions are blocked for want of a screen name ("the new layout used in code", "these two screens in navigation"). | OPEN items named |
| S98 | **Groups stay above home in navigation.** Home above groups would show a returning user figures scoped to a group they have not chosen, and adds a transition to the common path. The clarity that request was after comes from the switch instead (S97). | RECOMMENDED — needs sign-off |

# Rev 15 — 14 August 2026 · the invite and watcher set, drawn

Cumulative on rev 14 and everything before it. Drawing: `screens-invite-and-watcher.html` (eighteen frames). Full spec: `14-invite-and-watcher.md`. This closes the last three screens in the app with no current drawing — X2, C3 and X1. The mechanism behind all three was already on `main`; what existed was drawn against the chrome rev 9 replaced, so this is a redraw, not a rebuild.

| # | Change | Status |
|---|---|---|
| S77 | **X1 is a push, not a root.** The "watcher's install" does not exist: every install is a host install with a book in it, so a share link opens the club root and pushes the night on top of it, back → the club. **Supersedes the `X1 · Root, for a watcher's install` row in `09-navigation.md`.** A reader with no app gets the same body with X2's standalone header — a container swap, specified and not drawn. | FINAL |
| S78 | **X2, C3 and X1 are issued for build**, eighteen frames: **X2a** checking · **X2b** ready · **X2c** dead code · **X2d** typing a code · **C3a** invite · **C3b** claimed · **C3c** reset · **C3d** QR · **C3e** blocked · **X1a** watching live · **X1b** refused link · **X1c** a night that has ended. Both themes on the six that carry new layout; dark only where a strip or a string changes inside a paired layout. | FINAL |
| S79 | **The QR is not a sheet over a sheet.** C3d and C3c replace the sheet's content and keep the same close, per the depth rule — two sheets stays reserved for a player sheet raising an amount keypad. Swiping down from the QR leaves the whole sheet. | FINAL |
| S80 | **The dead invite is one string for four causes.** Unknown, spent, revoked and expired render an identical X2c — same layout, same sentence, same single control. Three build requirements: one error shape on the wire, constant response time across all four, no client-reachable cause in logs. Nothing on the screen names the group, host, inviter or code. **Security property, not a copy preference.** | FINAL / string PROPOSED |
| S81 | **A code is ten characters**, shown and entered as two groups of five, alphabet excluding **I, O, 0 and 1**. C3a's hero size and X2d's two fields both ride on this. | FINAL |
| S82 | **Nineteen strings are PROPOSED, not decided** — drafts written to make the frames drawable, listed with sources in the drawing and summarised in `14-invite-and-watcher.md` §3. Replacing any of them does not move the layout. Two primaries stand, not one: "Join the group" for the group-wide link, "Claim your place as Petr" for the per-player one. | **OPEN — needs sign-off** |
| S83 | **X1c re-derives the canonical night on the S62 split (by size of win): 110 / 31 / 29.** Nets become Dana +1,429 · Marek +526 · Lena +429 · Tomáš −500 · Ivo −780 · Petr −1,230, still summing to −126. This is the re-derivation S75 asked for and it **deliberately disagrees with the E-series frames**, which still draw 57/57/56. Sample data reconciled too: the host is **Marek** and the group is **the poker club** across all three screens; "Ivo" in `02-screens.md` §09 is sample data, and the string is `{host} added you as {name}`. | FINAL, supersedes S75's drawn figures |
| S84 | Two logic questions the drawing exposed: **does a per-player code expire** (the 7-day expiry is specified for the group link only; C3a is drawn with a binding sentence and no expiry line) and **what Reset does to an already-claimed code** (drawn live per S6, but a claim has already bound the member row to an account — if reset detaches it, that is an unbinding and needs its own copy). Plus four states named as not drawn: the watcher's browser container · the group-wide X2b · the signed-out C3e · a watcher on an empty table. | OPEN |

| S85 | **X2b carries a plan offer** — Regular $2.49/mo pre-selected, Full $9.99/mo beneath it, under a sentence built from the reader's own night count ("Four nights are already under your name. Free shows the last three."). All three tiers are one list **sorted ascending — Free $0, Regular, Full** — with Free drawn as already included (muted, outline check, `INCLUDED` tag) so the paid rows read as upgrades. **Nothing paid is ticked by default** — Free is the only marked row, and a plan is added by tapping rather than noticed and removed. Ascending rather than anchor-high because the reader already owns $0 and it is named in the sentence above the list, which is what carries the recommendation. Primary reads "Claim your place", becoming "Claim your place and start {plan}" once a paid row is tapped; the claim is identical either way. **This contradicts `01-product-logic.md` §4** ("Build none of it. No tiers, no payment, no locked states, no upsell sheets"): the block is **drawn but not scheduled** — the §4 seam holds, the policy module still returns `true` for everything in v1, and X2b ships without it until membership is in a build. Prices per the 12 Aug revision, not §4's older $4 / $9. | DRAWN, not scheduled |
| S86 | X2b's avatar removed — the card leads with the name and the nights. Nothing has been uploaded at claim time, so an initial in a box was decoration. | FINAL |
...
| # | Change | Status |
|---|---|---|
| S63 | **Group management is a section of its own**, admin-only, reached from Home and never from inside a live session: **GR1** club home (root) · **GR2** your groups · **GR3** new group · **GR4** the roster · **GR5** player edit · **GR6** invite this player · **GR7** settings · **GR8** money rules. | FINAL |
| S64 | **Settings are inherited, not entered.** Precedence is **this game → last game → club default → app default**. A night *copies* every rule when it opens; a later club change never alters a running night and never alters a settled one. | FINAL |
| S65 | **A game is started by adding players and their buy-ins.** The inherited rules appear as a summary with one row into the house rules; editing them is the rare path, not the default one. | FINAL |
| S66 | **Three standings per club:** admin · member · name only. A name-only player has no app and is seated, counted and settled exactly like a member. | FINAL |
| S67 | **Naming comes first, the invite second.** The admin adds a player by name and they play immediately; only then can a single-use link be sent from that player's own sheet, which hands them the name they already have and promotes them to member. No generic group invite, no pending-members list — an outstanding invite is a badge on a roster row. | FINAL |
| S68 | **Removing a player keeps their nights.** The ledger keeps what they already played; unsettled amounts stay on the night they came from. | FINAL |
| S69 | Leave-the-club and delete-the-club **confirm sheets are specified, not drawn** — they wait on the decisions below. | OPEN |
| S70 | Four decisions block the section: **two admins or one** (and how admin is handed over) · **leaving and deleting** with debt and with other people's nights · **the club book** as a shared ledger · **My stats vs My nights**, which currently both lead to the same history. | **OPEN** |

# Rev 12 — 13 August 2026 · the bill and the piggy bank, redrawn

Cumulative on rev 11 and everything before it. Board: `design/Bill and Piggy Bank.dc.html`. Build reference: `screens-bill-piggy-bank.html`. Full spec: `11-bill-and-piggy-bank.md`.

| # | Change | Status |
|---|---|---|
| S54 | **The B-series bill screens are dead.** B1–B6 on the night board are replaced by six screens in the Tonight chrome: **L1** the bill · **L2** add a spend · **L3** spend / edit · **L4** nothing on it · **L5** bill rules · **L6** piggy bank rules. | FINAL |
| S55 | **Nothing on the bill screens calculates a share.** While the game runs no result exists, so the screens carry the spends and the formula only. Winner shares, reimbursement amounts and the piggy bank total are computed once, at settle-up, against the counted table. | FINAL |
| S56 | **Adding a spend is one screen** — amount, an optional note, and who covered it. The time is stamped on save; there is no time field. | FINAL |
| S57 | **There is no type on a spend.** The chips above the note field are **prefills** (Food / Drinks / Venue) that write words into the note. The note itself is optional; an empty note shows as the amount alone. Nothing but the amount affects the arithmetic. | FINAL |
| S58 | **Covered by has four cases:** one player · several players, whose fronted amounts must sum to the spend or Save is blocked · the piggy bank, which takes no reimbursement · nobody yet, which leaves the spend unpaid with an amber tag. | FINAL |
| S59 | **The piggy bank is not on the bill.** It is its own screen off the house rules: on/off for tonight, the charge, **who collects** it, and a per-player off-tonight exception where the filled chip is the player switched off. | FINAL |
| S60 | **Admin only writes.** Everyone else gets the same screens with the buttons gone and the header meta reading "admin only". | FINAL |
| S61 | **A spend can be added after the count**, during settle-up, and recalculates every share and transfer. The settlement screen must say it changed. | FINAL |
| S62 | **Default split is by size of win** (was evenly between winners). ⚠ This contradicts the worked night in `04-money-math.md` (57/57/56 → 110/31/29) and every figure derived from it. No drawn screen is affected; the doc and the results screens that quote it are, until the canonical night is re-derived. | **OPEN** |

# Rev 11 — 13 August 2026 · Tonight's game is final

Cumulative on rev 10 and everything before it. Board: `design/Tonight Home.dc.html`. Full spec: `08-tonight-home.md` (rev 11).

| # | Change | Status |
|---|---|---|
| S50 | **H1 is the confirmed and only layout for Tonight's game.** Every other drawing of this screen in the older boards is dead. Build from `08-tonight-home.md` § *H1* and nothing else. | FINAL |
| S51 | **The running time is the live tag.** Green dot + elapsed time in green inside the 14 %-green pill; the word "LIVE" is gone. The **start time** sits at the right edge of the same title row and appears nowhere else. The club-name / elapsed meta line under the title is deleted from every frame. | FINAL |
| S52 | **One money card, no footer row.** "Total in play" as a separate row above the dock is gone; the figure is a `500 13px` line on the card's right side reading "$5,000 total in", above the seat count. **It hides when it equals what is on the table.** With nobody cashed out the seat count reads just "5 seated". | FINAL |
| S53 | **"On the table" is sentence case**, `600 12.5px`, no tracking, 8px above its value — the uppercase letterspaced eyebrow is not used for this label. | FINAL |
...
| S42 | **1C's summary is three figures:** Money in play (large, foreground) with **Bill** and **Piggy bank** grouped at the right in a smaller grey-label / red-value format, no minus signs. The buy-in and rebuy counts are not shown. | FINAL |
| S43 | **Each player row carries the whole calculation** as coloured tokens: `in 1,000` (red) · `out 1,300` (green) · `bill 61` · `piggy bank 15` (grey). A reimbursement rides **inside** its deduction — `bill 61 +170 back`, the `+170 back` in green — never as a separate token. Losers show `in`/`out` only, since deductions are charged to winners. | FINAL |
...
| Player | in | out | bill | piggy bank | reimbursed | net |
| --- | --- | --- | --- | --- | --- | --- |
...
- The bill is **170**, split winners-by-% (88 / 61 / 21); the piggy bank is **42**, 5% of each win (22 / 15 / 5).
- Nets sum to **−42**, not zero, because the bill returned to Marek and only the piggy bank actually left the table. A developer should assert this: `Σ nets = −(piggy bank + any rule not paid back to a person)`.
...
- **`C1b` input treatment specified** (S14): numeric buy-in, open chip row for rounding, disclosure rows for bills/piggy bank and settlement.
...
| S12 | C6 title is now **The piggy bank bank**. | FINAL |
...
- Money rules field labels: "Standard buy-in", "Food and drinks split, the piggy bank bank, fees", "Rounding".
...
**Worked example.** A 5% piggy bank on a night where Ivo ends −$540. Previously the rule existed and contributed $0 from Ivo, which made the rule's stated basis a lie. Now the rule cannot be authored that way: the UI disables *everyone* when the amount is a percentage, and the API returns a validation error for the pair.
...
**Rule.** On a night where nobody is in profit: percentage rules yield nothing, fixed fees and piggy bank charges to winners collect nothing and do not carry forward, and a bill backed by a real expense falls back to charging everyone at the table evenly.
...
**Worked example.** Dana +$430, with a $20 host fee (fixed, winners) and a 5% piggy bank on **net**.

- Fee first: 430 − 20 = 410, then 5% of 410 = 20.5 → **$21**. Dana keeps $389.
- Piggy bank first: 5% of 430 = 21.5 → **$22**, then fee $20. Dana keeps $388.
...
One state: filling in. Interactive: Group name (text, cursor shown) · Currency → picker · Stakes: small blind, big blind (numeric) and a straddle pick of No / Optional / Mandatory with a value field when not No · Money rules, three dashed **optional** fields — Rounding, Standard buy-in, Food and drinks split/piggy bank bank/fees — each opening its own editor · **Add** under *Who is in it* → name entry · **Create the group** → C2 roster.
...
Two states drawn: **nothing set** (dashed fields, placeholders naming the fallback — `$0`, "Nothing off the table", "Same night") and **filled in**. Interactive: Standard buy-in (numeric keypad) · Bills, piggy bank bank and fees → O3/O4 · Settlement due → C7 · Rounding chips, single pick, no navigation. **Skip** in the nav leaves every default in place; **Next · who is in it** → G2b.
...
Interactive: Name (text) · *Pays into the piggy bank* (toggle, on) · **Stats and history → Full stats** → that player's stats in the G4 layout · **Remove from the group** → confirm (undrawn) · **Save**.
