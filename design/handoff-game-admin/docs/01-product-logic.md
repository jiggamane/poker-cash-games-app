...
**The roster** [C2]. Everyone who has ever sat at this table, split into Regulars (three or more nights, or explicitly kept) and Occasional. Each row shows nights played and all-time net after the piggy bank. Nets shown here are *within this group only*.

**Editing a player** [C5]. Name, whether they pay into the piggy bank by default, and removal.
...
The designs carry a three-tier membership: Free (tonight, and the last three games), Regular $2.49/mo (every night in the book, the ones you missed included, and one night as host each month), Full $9.99/mo (runs games as often as they like). Regular's host night renews each billing period. Membership sits on the person, not the group, and follows them everywhere they play. Free can never run a game.
...
| `see_recent` | everyone (last three games) |
| `see_full_history` | Regular and above |
| `see_missed_nights` | Regular and above |
| `host_session` | Full, or Regular with this month's host night still free (opening or receiving a passed game) |
| `settle` | whoever admins the game, on any tier; follows `host_session`, never checked separately |
| `edit_group` | Full only |
...
**Rules are chosen on the screen itself, and inherited by default.** For a group that has played before, O1 carries the choice inline: **Keep last night's rules**, selected by default and naming them underneath ("Kitchen & drinks · Group piggy bank"), and **Change the rules** beside it — "edit, switch one off, or run with none" — which opens the money-rules screen. One button confirms every setting at once: **Open the table · 20:05**. A group's first night has nothing to inherit and goes to the rules step instead.
...
**Step 3 — money rules** [O3, O4]. Empty state offers three starting points — Food & drinks, Group piggy bank, Host fee — each of which creates a pre-filled rule. Configured rules show as cards with their settings as tags. A live preview sits under the list: *"If tonight ends like last Tuesday, $296 leaves the table: $170 back to Marek, $126 to the piggy bank."*

Rules can be skipped entirely; "Skip — no deductions" is offered at every step.

**Step 4 — the piggy bank tonight** [O7]. Optional, shown only when a piggy bank-type rule is active. Who it comes off for this night only. Three states per person: pays in, off tonight (with a reason line), and never pays in (a group-level setting, shown but not switchable here).
...
| Field | Options |
| --- | --- |
| **Name** | Free text, renameable by tapping the title ("Kitchen & drinks", "Group piggy bank") |
...
| **Destination** | A shared bill · a group piggy bank that carries over · a host fee · the next game's pot |
...
**The piggy bank carries over.** Its balance is a group-level running total across nights, shown on the rule card ("Balance $1,420").
...
**The dock at the foot** carries the two money actions only, sized by how often they are used: **Rebuy** (primary, ~2/3 width) and **Bill** (secondary, ~1/3). Rev 7 replaces the old Buy-in / Cash out pair.

Above the pair sits one collapsed row, **Table admin** (`seat · cash out · end`), which pulls up a drawer [N1b] holding the three infrequent controls: **Seat a player**, **Cash out a player**, and **End this poker night**. Ending is a **1.5s hold**, not a tap, and the drawer must be open first — two deliberate acts before a night can close. The Table admin row is a 46px target with 14px of clearance above Rebuy, so a thumb reaching for it cannot land on Rebuy instead.

Cashing out is reachable two ways: from the drawer (pick a player, N8) and directly on a player [N1c].
...
- **Rebuy** [N6]: the player's **last rebuy amount is the default** (see below), then double (×2), quadruple (×4), and Custom. The header states which rebuy this is ("REBUY · 3RD") and what they are already in for.

**The default rebuy amount is that player's last rebuy on this night.** Tables drift from the standard buy-in as the night goes on — once someone rebuys $500 at a $200 table, their next rebuy is almost always $500 again, and the old behaviour made the host retype it every time.

- Resolution order, per player, per session: **last rebuy this session → the session's standard buy-in → the group's default buy-in.**
- It is a **per-player** memory, not a table-wide one. Petr rebuying $500 does not change Ivo's default.
- A **voided** rebuy stops counting; the default falls back to the one before it. A **corrected** rebuy counts at its corrected amount.
- **Not surfaced in the interface.** The button reads "Rebuy $500" and nothing explains where the figure came from; a host who wants a different number taps *Other amount*. The resolution order above is a dev-side rule — keep it in code and in the entry log, out of the UI.
- First buy-ins are unaffected: N5 keeps the group default.
...
## 8. The bill and the piggy bank during play

Reached from the live session by the info icon on the header [B1]. The house rules sheet restates the group's rules as they apply tonight, with the piggy bank's estimate at the current standing ("≈ $126").
...
A bill has fronters, not a collector. The collector field on a rule is for money that accumulates — the piggy bank, a host fee.

**The piggy bank can be switched off for someone mid-night** [B6]. Tonight only, and it changes *who it comes off*, never the rate. Nothing already logged changes — the piggy bank is worked out at settle-up, so this is always safe.
...
**My stats** [G4]: a person's own record. Across all groups and per group: total net, best night, worst night, nights in the black, all after piggy bank and bills. Only you see this page. Inside a group, members see your net *in that group* and nothing else.
