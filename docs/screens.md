...
There is a second version of that hole, and B6 is it: a route can be in every
...
them fixed and one of them not, which is B6.
...
the rule pass measured for seventeen runs while the bug in B6 sat on it. The
