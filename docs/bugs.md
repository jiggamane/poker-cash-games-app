...
*Numbered **B91** on merge, not when written — it was B89 on its branch, and
the entry above it was written in the same commit as B90 and keeps that number.
The sixth collision this file has recorded, and the second in a day: a session
hardening `check:ui` merged its own B89 while this one was in that very gate.
Same resolution as every time — **the entry already on `main` keeps the
number**, and this one moved up past it, with its references: the fifteen `B89`
mentions across `who.ts`, `sync.ts`, `backupPump.ts`, `useSession.ts`,
`settings.tsx`, `invite.tsx`, `auth-callback.tsx`, `who.test.ts`,
`accountLine.test.ts`, `scripts/ui-audit.mjs`, `docs/screens.md`,
`docs/storage-and-sync.md` and B90 above moved with it. The commit message on the branch still
says B89, because it was written before `main` moved and a pushed message is not
worth rewriting; the file is the record, and the file says B91.*
...
(B91 had no state for it, so there was no copy either), `Saved on this phone`
...
The doc now says what the code does. **All three were built the next day** —
`backupPump.ts` drains when the app comes to the foreground, when somebody signs
...
### B82 — Sessions lost the end of its meta line at the reader's larger text
...
Found       15 Sept, by the meta-line-truncated pass written for B81, on its
...
*Written as B80 and renumbered in the merge alongside B81 — see the note there.*

**Found by the check rather than on a phone, which is the whole point of
writing one.** B81 was photographed; this was two points over on a screen
...
between this line and B81's, which was rebuilt out of terms that cannot grow —
and it is the reason this one is watched by a check now.

### B81 — the past session's meta line still truncated, one term later
...
*Written as B79 and renumbered in the merge — the sixth time this file has
recorded that, and the second today. B78 was the highest on `main` when this
branch forked; a session on the book merged its own B79 and B80 while this one
was being checked, so those keep the numbers and these two moved up. The rule at
the top of this file, applied.*
