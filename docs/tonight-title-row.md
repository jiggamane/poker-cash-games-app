...
**The title row is the drawn row again — title, then tag — and the start time
moved into the On-the-table card**, as a third line in the right-hand meta
stack under "5 seated · 1 out".
...
happening, which is what S51 made it. The start time never changes and is read
once, so it belongs with the night's other fixed facts rather than in the
chrome. Putting it there costs nothing anywhere:
...
So the fix gives 12pt of table back rather than spending header height on the
problem, and the tag sits beside the title exactly as drawn.

This does break **S51**, which put the start time at the right edge of the
title row. What S51 was protecting still holds: the start time appears exactly
once, and the running time is still the live tag. `Screen.tsx` now says in its
`trailing` prop that a badge and a trailing text must not share a row — that
pairing is what broke the title, and Tonight was the only screen doing it.

## The three layouts this was chosen over

All kept the start time in the chrome, and all were rendered from the app
before being turned down:

- **Both clocks stacked at the right edge.** The tag over "started 16:37",
  sharing a right margin. Works everywhere and is the same shape at every
  width, but costs 12pt of header height and takes the tag away from the title.
  This one was on `main` briefly.
