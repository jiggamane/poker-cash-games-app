...
six-digit code came off the second stage (B88), which leaves a screen whose
...
re-think of anything else. Read `docs/bugs.md` B88 first if you want the
...
- **No code field, and no `{{ .Token }}` in the email template**, unless custom
  ...
  change. That is B88, locked by the same test.
