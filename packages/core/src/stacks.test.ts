...
describe('a rounded night cannot be drawn in four columns', () => {
  it('says so, because the step is a fifth term with no column', () => {
    /* `game + food + piggy` is short by whatever the step moved, so the layout
       that puts the whole formula on one row cannot be used. */
    expect(columnsFit(at('dollars'))).toBe(true);
    expect(columnsFit(at('tens'))).toBe(false);
  });

  it('and the columns still add up on a night that did not round', () => {
    for (const r of resultColumns(at('dollars'))) {
      expect(r.game + r.food + r.piggy).toBe(r.net);
    }
  });
});
