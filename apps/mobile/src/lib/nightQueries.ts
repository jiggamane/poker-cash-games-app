/**
 * The queries whose exact wording is a money-adjacent decision.
 *
 * `nightStore.ts` cannot be imported outside a phone — it pulls in React,
 * react-native and expo-sqlite — so anything in it that could be wrong in a
 * way nobody would notice lives here instead, as a value, and is tested
 * against a real SQLite. `pullReads.ts` and `syncRows.ts` exist for the same
