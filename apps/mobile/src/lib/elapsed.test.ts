...
describe('the elapsed label', () => {
  it('reads the way both screens draw it', () => {
    expect(elapsedLabel(START, at(0))).toBe('0h 00m');
  ...
  it('never goes negative on a clock that disagrees with the night', () => {
    ...
    expect(elapsedLabel(START, at(-5 * MINUTE))).toBe('0h 00m');
  });
...
describe('when the label next changes', () => {
  it('waits to the exact half-minute boundary the rounding turns on', () => {
    // The label rounds, so it flips at 30s, 90s, 150s… not on the minute.
    expect(msUntilNextLabelChange(START, at(0))).toBe(30_000);
    expect(msUntilNextLabelChange(START, at(29_000))).toBe(1_000);
    expect(msUntilNextLabelChange(START, at(31_000))).toBe(59_000);
  });

  it('never schedules a zero-delay timer', () => {
    // A boundary landing exactly on now would otherwise spin the event loop.
    expect(msUntilNextLabelChange(START, at(30_000))).toBe(MINUTE);
